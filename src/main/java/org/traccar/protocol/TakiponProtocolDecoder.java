...
public class TakiponProtocolDecoder extends BaseProtocolDecoder {
    ...
    public static boolean decodeGps(
        ...
        position.set(Position.KEY_ROAMING, BitUtil.check(flags, 13)); // GPS real-time/differential positioning
        position.set(Position.KEY_SATELLITES_VISIBLE, BitUtil.check(flags, 12)); // GPS having been positioning or not
    ...
    private Object decodeBasic(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (type == MSG_LOGIN) {
            ...
            if (type == MSG_GPS_LBS_1) {
                if (type == MSG_GPS_LBS_1) {
                    // Login sonrası ilk konumları filtrele
                    if (deviceSession.shouldSkipFirstLocation()) {
                        sendResponse(channel, false, type, buf.getShort(buf.writerIndex() - 6), null);
                        LOGGER.info("Giriş sonrası ilk konum filtrelendi");
