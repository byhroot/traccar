...
public class IdleEventHandler extends BaseEventHandler {
    ...
    private static final long DEFAULT_IDLE_THRESHOLD = 20 * 60 * 1000; // 30 dakika

    private final CacheManager cacheManager;
    private final Storage storage;
    private final long idleThreshold;
    private final boolean active; // işlem aktif mi?
    ...
    public void onPosition(Position position, Callback callback) {
        // 📌 Eğer handler devre dışıysa hiçbir işlem yapmadan çık
        if (!active) {
        ...
        if (!ignition || motion) {
            return; // kontak kapalıysa alarmla ilgilenmeye gerek yok
        }
        
        // Last position bilgileri
        ...
        // Kontak açık ama idle başlangıcı reset gerekirse:
        boolean resetIdle = false;
        if (!lastIgnition && ignition) { // kontak kapalı → açık
            resetIdle = true;
        } else if (lastMotion && !motion) { // araç durdu
            resetIdle = true;
        } else if (lastSpeed > 1.0 && speed <= 1.0) { // hız sıfırlandı
            resetIdle = true;
        }
