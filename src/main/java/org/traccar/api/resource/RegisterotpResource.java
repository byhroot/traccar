...
public class RegisterotpResource extends BaseResource {
    ...
    public Map<String, Object> sendOtp(@QueryParam("phone") String phone) throws StorageException {
        ...
        List<Long> timestamps = smsRequestLog.computeIfAbsent(clientIp, k -> Collections.synchronizedList(new ArrayList<>()));
        ...
        try {
            actionLogger.other(request, 0, "sms", "OTP", 1 , "Phone:"+ phone, "OTP SMS Gönderildi");
        } catch (Exception e){
    ...
    public Map<String, Object> verifyOtp(@PathParam("id") String id, Map<String, String> input) throws StorageException {
        ...
        Request requestAdmins = new Request(
            new Columns.All(), // Tüm kolonlar
            new Condition.Equals("administrator", true) // administrator alanı true olanları al
        );

        List<User> adminUsers = storage.getObjects(User.class, requestAdmins);
        // Notification gönderme
        for (User admin : adminUsers) {
            try {
                String subject = "Yeni OTP Doğrulama";
                String body = "Kullanıcı bir OTP doğrulaması gerçekleştirdi: ID numarası:";
        ...
        try {
            actionLogger.other(request, 0, "sms", "OTP", 1 ,"test", "OTP SMS Gönderildi");
        } catch (Exception e){
            LOGGER.info("SMS DB loglama hatası oldu",e);
        }
